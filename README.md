...
- High-contrast native workbench treatment for composer icons, model menus, activity labels, browser and file-preview hosts, file-change cards, Markdown tables, and right-side thread summary panels.
- Native titlebar safe-area reservation keeps left navigation controls below the window chrome and clickable.
...
The injected stylesheet also corrects native desktop components that retain light-mode tokens after the theme changes: composer icon buttons and their SVGs, model menus, thinking and activity labels, browser and file-preview hosts, file-change cards, Markdown roots and tables, and the right-side environment, change, and source summary panel. Table headers and cells receive theme-derived surfaces, readable foreground text, and visible separators. The summary panel's headings, metadata, icons, border, and surface follow the active theme. The left navigation reserves the native titlebar safe area so its top controls remain below the window chrome and clickable. The rules are scoped to the relevant renderer surfaces so semantic controls such as stop actions keep their native state treatment.
...
The native adapter uses the Settings shortcut first, then a visible account or
profile menu when a build places Settings there.
...
<img src="codex-skin-skill-qrcode.jpg" alt="Contact QR code" width="188">
