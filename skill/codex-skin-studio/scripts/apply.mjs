...
const HERO_URL_TOKEN = "__CODEX_SKIN_HERO_URL__";
...
const MAIN_TARGET_PROBE = `(() => { const root = document.getElementById("root"); const shell = document.querySelector(".app-shell-left-panel, .main-surface, .browser-main-surface, .composer-surface-chrome"); const title = document.title || ""; const url = location.href; const windowsCodex = /codex/i.test(title) && url.startsWith("app://-/index.html"); return { main: Boolean(root && (shell || windowsCodex)), root: Boolean(root), shell: Boolean(shell || windowsCodex), title, url }; })()`;
...
function styleExpression(theme, hero, logo = null, polaroid = null, themes = []) {
  const style = css(theme, HERO_URL_TOKEN, logo, polaroid);
