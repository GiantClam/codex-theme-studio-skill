...
- 输入栏图标、模型菜单、思考与活动标签、文件变更卡和 Markdown 表格均有高对比度原生工作台处理。
...
注入样式还会修复主题切换后仍保留浅色模式 token 的原生桌面组件：输入栏图标按钮及其 SVG、模型菜单、思考与活动标签、文件变更卡和 Markdown 表格。表头与单元格使用主题派生表面、可读前景色和可见分隔线。规则仅作用于对应的 Renderer 区域，因此停止等具有状态语义的原生操作仍保留自身样式。
