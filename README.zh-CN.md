...
- 输入栏图标、模型菜单、思考与活动标签、浏览器和文件预览宿主、文件变更卡、Markdown 表格以及右侧线程摘要面板均有高对比度原生工作台处理。
- 左侧导航会预留原生标题栏安全区，避免顶部导航控件被窗口 chrome 覆盖而无法点击。
...
注入样式还会修复主题切换后仍保留浅色模式 token 的原生桌面组件：输入栏图标按钮及其 SVG、模型菜单、思考与活动标签、浏览器和文件预览宿主、文件变更卡、Markdown 根容器和表格，以及右侧的环境信息、变更和来源摘要面板。表头与单元格使用主题派生表面、可读前景色和可见分隔线；摘要面板的标题、元数据、图标、边框和表面均会跟随当前主题。左侧导航会为原生标题栏预留安全区，使顶部控件保持在窗口 chrome 下方并可点击。规则仅作用于对应的 Renderer 区域，因此停止等具有状态语义的原生操作仍保留自身样式。
