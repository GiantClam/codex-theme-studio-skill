...
test("emits dark CSS variables and a quoted hero URL", () => {
  ...
  assert.match(value, /outline-color: var\(--codex-skin-accent\) !important/);
  assert.match(value, /\[class\*="ModelPickerTrigger" i\]/);
  assert.match(value, /body \:is\(\[role="menu"\], \[role="listbox"\]\) \{/);
  assert.match(value, /body \:is\(\[role="menu"\], \[role="listbox"\]\) \:is\(span, div, p, button, \[role="menuitem"\], \[role="option"\]\)/);
  assert.match(value, /ComposerLayoutRoot" i\] button\.aspect-square \{/);
  assert.match(value, /ComposerLayoutRoot" i\] button\.aspect-square svg \{/);
  assert.match(value, /button\.aspect-square:hover:not\(:disabled\)/);
  assert.match(value, /\.text-fade-truncate/);
  assert.match(value, /\[contenteditable="true"\]/);
  assert.match(value, /\.loading-shimmer-pure-text \{/);
  assert.match(value, /-webkit-text-fill-color: var\(--codex-skin-text\) !important/);
  assert.match(value, /background: none !important;/);
